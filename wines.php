...
<div id="wrapper">
  <?php
    ...
    function displayWinesList($connection, $query, $wineName, $wineType, $regionName, $max_price)
    {
     ...
     {
         // ... print out a header
       print "<br/>{$rowsFound} wines found matching your criteria<br/><br/>";
       ...
       print "<br/><h2>No such wine was found.</h2>";
     }
  ...
  // Secure the user parameter $min_price
  //$min_price = mysqlclean($_GET, "min_price", 30, $connection);

  // Secure the user parameter $max_price
  $max_price = mysqlclean($_GET, "max_price", 30, $connection);

  // Price validation - ensure min is less than or equal to max
  /*if (isset($min_price) && isset($max_price) && $min_price != "" && $max_price != "" && $min_price > $max_price)
  {
    $tmp = $min_price;
    $min_price = $max_price;
    $max_price = $tmp;
  ...
  $query = "SELECT wine.wine_id, wine_name, description, year, winery_name, wine_type.wine_type, cost
   ...
   // as an AND clause ...
   if (isset($wineType) && $wineType != "All" && $wineType != "")
     $query .= " AND wine_type.wine_type COLLATE UTF8_GENERAL_CI LIKE \"%{$wineType}%\"";
   ...
   // ... then, if the user has specified a min_price, add the min_price
   // as an AND clause ...
   //if (isset($min_price) && $min_price != "")
   //  $query .= " AND cost >= \"{$min_price}\"";

   // ... then, if the user has specified a max_price, add the max_price
   // as an AND clause ...
  ...
  //print "\n<BR>Query=\"{$query}\"<BR>";
   ...
   displayWinesList($connection, $query, $wineName, $wineType, $regionName, $max_price);
